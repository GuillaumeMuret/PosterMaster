...
public class EseoGetMyJuryListRequest extends EseoBaseRequest {

    public static final String REQUEST_GET_MY_JURY_LIST ="REQUEST_GET_MY_JURY_LIST";

    public static boolean isRequestRunning = false;

    public EseoGetMyJuryListRequest(String requestId, boolean isShowLoadingDialog, Context context, Users appUser) {
        ...
        try{
            if (webMessage.getResult().equals(ProtocolVocabulary.JSON_RESULT_VALUE_OK)) {
                ...
                for(int i=0;i<webMessage.getJuries().length;i++){
                    ...
                    JuryMembers.updateJuryMembersReceivedFromEseo(context, webMessage.getJuries()[i].getIdJury());
    ...
    protected Call createCall(Object interfaceObject) {
        Call call = ((EseoService)interfaceObject).getMyJuryList(
                ProtocolVocabulary.PROCESS_LIST_MY_JURIES_INFO,
