...
public class EseoGetMyProjectListRequest extends EseoBaseRequest {

    public static final String REQUEST_GET_MY_PROJECT_LIST ="REQUEST_GET_MY_PROJECT_LIST";

    public static boolean isRequestRunning = false;

    public EseoGetMyProjectListRequest(String requestId, boolean isShowLoadingDialog, Context context, Users appUser) {
        ...
        try{
            if (webMessage.getResult().equals(ProtocolVocabulary.JSON_RESULT_VALUE_OK)) {
                ...
                for(int i=0;i<webMessage.getProjects().length;i++){
                    Supervisors.updateMyProjectReceivedFromEseo(context,webMessage.getProjects()[i].getProjectId(),appUsers.getIdUser());
    ...
    protected Call createCall(Object interfaceObject) {
        Call call = ((EseoService)interfaceObject).getMyProjectList(
                ProtocolVocabulary.PROCESS_LIST_MY_PROJECT,
